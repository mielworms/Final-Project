from pygame import mixer
from tkinter import Tk
from tkinter import Label
from tkinter import Button
from tkinter import filedialog

current_volume = float(0.5)

#Functions
def play_song():
    filename =filedialog.askopenfilename(initialdir="C:/",title="Select a file!!")
    current_song = filename
    song_title = filename.split("/")
    song_title = song_title[-1]
    print(song_title)

    try:
        mixer.init()
        mixer.music.load(current_song)
        mixer.music.set_volume(current_volume)
        mixer.music.play()
        song_title_label.config(fg="green",text="Now Playing: " + str(song_title))
        volume_label.config(fg="green",text="Volume: " + str(current_volume))
    except Exception as e:
        print(e)
        song_title_label.config(fg="red", text="Error playing track")
#Main Screen
master = Tk()
master.title("Music Player")

#Labels
Label(master,text="Custom Music Player",font=("Arial",20),fg="black").grid(sticky="N",row=0,padx=150)
Label(master,text="Select a song :D",font=("Arial",15),fg="red").grid(sticky="N",row=1)
Label(master,text="Volume",font=("Arial",12),fg="blue").grid(sticky="N",row=4)
song_title_label = Label(master,font=("Arial",12))
song_title_label.grid(stick="N",row=3)
volume_label = Label(master,font=("Arial",12))
volume_label.grid(sticky="N",row=5)

#Buttons
Button(master,text="Select Song", font=("Arial",12),command=play_song).grid(row=2,sticky="N")
Button(master,text="Pause", font=("Arial",12)).grid(row=3,sticky="E")
Button(master,text="Resume", font=("Arial",12)).grid(row=3,sticky="W")
Button(master,text="-", font=("Arial",12),width=5).grid(row=5,sticky="w")
Button(master,text="+", font=("Arial",12),width=5).grid(row=5,sticky="E")

master.mainloop()